...
export class SqlServerPagingQueryProvider extends AbstractSqlPagingQueryProvider {
  ...
  override generateRemainingPagesQuery(pageSize: number): SQL {
    const topClause = this.buildTopClause(pageSize);
    if (StringUtils.hasText(this.groupClause)) {
      return this.buildSql(this.generateGroupedTopSqlQuery(topClause));
    }

    return this.buildSql(this.generateTopSqlQuery(true, topClause));
  }

  private buildTopClause(pageSize: number): string {
    return `TOP ${pageSize}`;
  }
  ...
  private generateTopSqlQuery(remainingPageQuery: boolean, topClause: string): string {
    ...
    query += ` ORDER BY ${this.buildSortClause()}`;
    return query;
  }

  private generateGroupedTopSqlQuery(topClause: string): string {
    return `SELECT ${topClause} * FROM (SELECT ${this.selectClause} FROM ${this.fromClause}${this.whereClause == null ? "" : ` WHERE ${this.whereClause}`}${this.groupClause == null ? "" : ` GROUP BY ${this.groupClause}`}) AS MAIN_QRY WHERE ${this.buildSortConditions()} ORDER BY ${this.buildSortClause()}`;
  }

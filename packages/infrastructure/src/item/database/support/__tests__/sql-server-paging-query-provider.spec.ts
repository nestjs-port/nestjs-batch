...
import { SqlServerPagingQueryProvider } from "../sql-server-paging-query-provider.js";
import { runAbstractSqlPagingQueryProviderTests } from "../abstract-sql-paging-query-provider-tests.js";

describe("SqlServerPagingQueryProvider", () => {
  runAbstractSqlPagingQueryProviderTests(
    () => new SqlServerPagingQueryProvider(),
    {
      ...
      expectedFirstPageQueryWithGroupBy:
        "SELECT TOP 100 id, name, age FROM foo WHERE bar = 1 GROUP BY dep ORDER BY id ASC",
      expectedRemainingPagesQueryWithGroupBy:
        "SELECT TOP 100 * FROM (SELECT id, name, age FROM foo WHERE bar = 1 GROUP BY dep) AS MAIN_QRY WHERE ((id > ?)) ORDER BY id ASC",
    },
  );
});

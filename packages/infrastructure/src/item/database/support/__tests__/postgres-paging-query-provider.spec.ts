...
import { PostgresPagingQueryProvider } from "../postgres-paging-query-provider.js";
import { runAbstractSqlPagingQueryProviderTests } from "../abstract-sql-paging-query-provider-tests.js";

describe("PostgresPagingQueryProvider", () => {
  runAbstractSqlPagingQueryProviderTests(
    () => new PostgresPagingQueryProvider(),
    {
      ...
      configureGroupClause(provider) {
        provider.setGroupClause("id, dep");
      },
      expectedFirstPageQueryWithGroupBy:
        "SELECT id, name, age FROM foo WHERE bar = 1 GROUP BY id, dep ORDER BY id ASC LIMIT 100",
      expectedRemainingPagesQueryWithGroupBy:
        "SELECT * FROM (SELECT id, name, age FROM foo WHERE bar = 1 GROUP BY id, dep) AS MAIN_QRY WHERE ((id > ?)) ORDER BY id ASC LIMIT 100",
    },
  );
});
